...
def make_wavenumbers(
    ...
    *,
    ...
    k = 2.0 * torch.pi * torch.fft.rfftfreq(nx, d=dx, device=device)
    return k.to(dtype=dtype)


def make_dealias_mask(
    nx: int,
    device: torch.device,
    dtype: torch.dtype,
) -> torch.Tensor:
    if nx <= 1:
        raise ValueError("nx must be >= 2")
    cutoff = nx // 3
    freqs = torch.arange(nx // 2 + 1, device=device)
    return (freqs <= cutoff).to(dtype=dtype)


def _validate_fourier_width(
    value: torch.Tensor,
    *,
    name: str,
    nx: int,
    ...
    if nx <= 1:
        raise ValueError("nx must be >= 2")
    expected_width = nx // 2 + 1
    if value.ndim < 1 or value.shape[-1] != expected_width:
        shape = tuple(value.shape)
        raise ValueError(
            f"{name} Fourier width must be nx // 2 + 1 = "
            f"{expected_width} for nx={nx}, got shape {shape}"
        )
...
def _apply_dealias(
    ...
    *,
    ...
    _validate_fourier_width(u_hat, name="u_hat", nx=nx)
    if mask is not None:
        _validate_fourier_width(mask, name="mask", nx=nx)
    if not dealias:
        return u_hat
    if mask is None:
        mask = make_dealias_mask(nx, u_hat.device, u_hat.real.dtype)
    return u_hat * mask.to(device=u_hat.device, dtype=u_hat.real.dtype)
...
def burgers_nonlinear_hat(
    ...
    *,
    ...
    _validate_fourier_width(u_hat, name="u_hat", nx=nx)
    _validate_fourier_width(k, name="k", nx=nx)
    if mask is not None:
        _validate_fourier_width(mask, name="mask", nx=nx)
    u = torch.fft.irfft(u_hat, n=nx, dim=-1, norm="forward")
    n_hat = -0.5j * b * k * torch.fft.rfft(
        u * u,
        n=nx,
        dim=-1,
        norm="forward",
    )
    return _apply_dealias(n_hat, nx=nx, dealias=dealias, mask=mask)
...
def burgers_split_step_outer(
    ...
    *,
    ...
    if mask is not None:
        _validate_fourier_width(mask, name="mask", nx=nx)
    if forcing_hat is not None and forcing_hat.shape != u_hat.shape:
        raise ValueError(
            f"forcing_hat must have shape {tuple(u_hat.shape)}, "
            f"got {tuple(forcing_hat.shape)}"
        )
...
def simulate_burgers_split_step(
    ...
    if nx <= 1:
        raise ValueError("nx must be >= 2")
    k = make_wavenumbers(nx, z0.device, z0.dtype, domain_length=domain_length)
    mask = make_dealias_mask(nx, z0.device, z0.dtype) if dealias else None
    ...
    if forcing is not None:
        ...
        forcing_hat = torch.fft.rfft(
            forcing,
            n=nx,
            dim=-1,
            norm="forward",
        )
        forcing_hat = _apply_dealias(
            forcing_hat,
            nx=nx,
            dealias=dealias,
            mask=mask,
    ...
    u_hat = torch.fft.rfft(z0, n=nx, dim=-1, norm="forward")
    u_hat = _apply_dealias(
        u_hat,
        nx=nx,
        dealias=dealias,
        mask=mask,
